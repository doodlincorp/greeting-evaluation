...
enum class EvaluationScoreCalculationType : CheckEvaluationCalculation {
    MANUAL {
        override fun checkWeightSize(weightSize: Int) {
            // 가중치가 존재해선 안됨
            if (weightSize > 0) {
                throw BadRequestException(doodlin.greeting.evaluation.business.domain.errors.EvaluationException.EVALUATION_WEIGHT_ONLY_WEIGHT_SCORE.message)
            }
        }
    ...
    AVERAGE_SCORE {
        ...
        override fun checkDetailEvaluationSize(detailEvaluationSize: Int) {
            ...
            if (detailEvaluationSize == 0) {
                throw BadRequestException(doodlin.greeting.evaluation.business.domain.errors.EvaluationException.MUST_HAVE_DETAIL_EVALUATION_EXCEPT_MANUAL.message)
            }
        }
    ...
    WEIGHT_SCORE {
        ...
        override fun checkWeightScore(notValidWeightScore: Boolean) {
            ...
            if (notValidWeightScore) {
                throw BadRequestException(doodlin.greeting.evaluation.business.domain.errors.EvaluationException.NOT_VALID_SCORE_AND_WEIGHT.message)
            }
        }
        ...
        override fun checkWeightSumAndSize(weightSum: Int, weightSize: Int) {
            ...
            if (
                ...
                throw BadRequestException(doodlin.greeting.evaluation.business.domain.errors.EvaluationException.INVALID_EVALUATION_WEIGHT.message)
            }
        }
    }
}
